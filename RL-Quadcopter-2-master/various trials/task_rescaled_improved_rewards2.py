...
class Task():
    ...
    def get_delta_distance_reward_new(self, previous_pos, new_pos):
        ...
        if dist_old<=2.5*self.target_margin:
            ...
            reward=-7.5*((dist_old/self.total_distance)**0.2)/100 # The idea for this came from https://www.youtube.com/watch?v=0R3PnJEisqk
    ...
    def get_delta_distance_reward_old(self, previous_pos, new_pos):
        """Uses the evolution of pose with respect to the target to return a reward.
        1) positive reward if the distance to the target has decreased
        2) negative reward if the distance to the target has increased
        """
    ...
    def get_cosine_reward(self, previous_pos, new_pos):
        ...
        #dist_old=np.dot(previous_pos-self.target_pos,previous_pos-self.target_pos)**(0.5)
        if dist_new<=0.1:
            reward=250*self.total_distance*self.runtime
            print("\nSUCESS!!!!!")
            self.success=1
                #if cosine>=0:
                #reward=self.runtime
        elif dist_new<=0.5:
            reward=100*self.total_distance*self.runtime
            print("$$$$$ Almost there $$$$$")
            #if cosine<0.0:
            #   reward=-10*cosine             
        elif dist_new<=1:
            reward=50*self.total_distance*self.runtime
            print("$$$$$ Getting there $$$$$")
            #if cosine <0.0:
            #   reward=-5*cosine
        elif self.hit_the_bounds(new_pos):
            reward=-10*self.total_distance*self.runtime/(3)
            #if cosine>=0:
                #reward=1/3
        else:
            reward=cosine
    ...
    def get_reward(self, previous_pos, new_pos, cos=0):
        if cos==0:
            reward= self.get_delta_distance_reward_new(previous_pos, new_pos)
        else:
